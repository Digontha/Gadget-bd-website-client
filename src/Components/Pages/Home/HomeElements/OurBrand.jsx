...
const OurBrand = ({ data }) => {
    ...
    const handleData = (e) => {
        ...
        const filterData = data?.filter(item => item.brand_name.toLowerCase().includes(brandName.toLowerCase()));
                        ...
                        <Link to={`/brand/${item.brand}`}>
                            <div onClick={handleData} className="card card-compact  bg-base-100 shadow-xl">
                                ...
                                <div className="card-body">
                                    <h2 className="font-extrabold text-xl text-center">{item.brand}</h2>
                                </div>
                            </div>
                        </Link>
                    </div>)
